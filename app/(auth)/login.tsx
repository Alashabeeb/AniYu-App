...
export default function LoginScreen() {
  ...
  const handleSocialLoginClick = async (provider: 'google' | 'apple') => {
    ...
    try {
      ...
      if (error.code !== '12501') showAlert('error', 'Login Failed', getFriendlyErrorMessage(error));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={[styles.container, { backgroundColor: theme.background }]}>
      <View style={styles.content}>
        ...
        <TouchableOpacity style={[styles.button, { backgroundColor: theme.tint }]} onPress={handleLoginClick} disabled={loading}>
      ...
      <CustomAlert visible={alertConfig.visible} type={alertConfig.type} title={alertConfig.title} message={alertConfig.message} onClose={() => setAlertConfig(prev => ({ ...prev, visible: false }))} />
    </KeyboardAvoidingView>
  );
}
